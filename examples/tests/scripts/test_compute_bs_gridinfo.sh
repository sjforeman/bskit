...
export STARTI=0
export ENDI=200
export OUTFILE=${OUT_DIR}Lbox1000_512_kf_3kf_3lowkbins.dat
...
mpirun -np 8 python ${SCRIPT_DIR}measure/measure_bs_fast.py bigfile_grid $SNAP $NGRID $NGRIDCIC $LBOX $STARTI $ENDI $OUTFILE --k_bin_info $KMIN $KMAX $DK --high_k_bin_info $NUM_LOWK_BINS $DK_HIGH --triangle_type all --meas_type grid_info
