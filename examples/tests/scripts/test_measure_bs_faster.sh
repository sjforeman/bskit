...
export OUTFILE=${OUT_DIR}${FILE_ROOT}_unnormbs_kf_3kf_3lowbins.dat
...
mpirun -np 8 python ${SCRIPT_DIR}measure/measure_bs_fast.py bigfile_grid $SNAP $NGRID $NGRIDCIC $LBOX $STARTI $ENDI $OUTFILE --k_bin_info $KMIN $KMAX $DK --high_k_bin_info $NUM_LOWK_BINS $DK_HIGH  --triangle_type all
