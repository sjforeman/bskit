...
The --meas_type argument specifies whether to measure the bispectrum value along with
the mean k1,k2,k3 values and number of triangles in each bin ('full'), just the
bispectrum value (unnormalized by the number of triangles) ('unnorm_b_value'), or just
the mean k1,k2,k3 and number of triangles ('grid_info').

This script calls the slower but less memory-intensive bispectrum measurement routine
in bskit - for measurements of many triangles, the faster routine is recommended.
"""
...
parser.add_argument('--meas_type',
                    choices=['full','grid_info','unnorm_b_value'],
...
# Measure bispectrum at triangle indices starting at start_i and ending at end_i,
# writing to out_file
fftb.measure_bispectrum(imin=start_i,imax=end_i,out_file=out_file,verbose=0,meas_type=meas_type)
