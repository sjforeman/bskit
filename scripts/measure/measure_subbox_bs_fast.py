"""Measure auto or cross bispectrum from subboxes of input snapshot(s).
...
Separate output files will be generated for each subbox.
"""
...
parser.add_argument('start_subbox_index',type=int,
                    help='index of first subbox to use')
parser.add_argument('end_subbox_index',type=int,
                    help='index of last subbox to use')
parser.add_argument('out_file_prefix',
                    help='prefix of bispectrum output files')
...
parser.add_argument('--meas_type',
                    ...
                    help='type of measurement to do (full, grid_info, or unnorm_b_value')
...
start_subbox_ind = args.start_subbox_index
end_subbox_ind = args.end_subbox_index
...
# Set up dicts for catalogs, meshes, and fields
cat = {}
mesh = {}
field = {}
for i in range(3):
...
# Loop over subboxes
for ind in range(start_subbox_ind,end_subbox_ind+1):
    # Get tuple of (x,y,z) coordinate subregion indices from single subbox index
    multi = bskit.subbox_index_to_multiindex(ind,nsub_per_side)
    
    out_file = out_file_prefix + '_subbox%d.dat' % ind
    print_status(comm,start_time,'About to measure B from subbox %d' % ind)

    # Fetch mesh corresponding to subbox from full field
    for n in range(num_fields):
        sub_mesh[n] = bskit.field_subbox_pm(multi,nsub_per_side,field[n])

    # Initialize FFTBispectrum object
    if args.k_bin_file:
        # If k bin file has been specified, initialize FFTBispectrum object with the
        # corresponding binning scheme, accounting for Nmesh and BoxSize of subbox
        fftb = bskit.FFTBispectrum(sub_mesh[0],Nmesh=Nmesh/nsub_per_side,
                                   BoxSize=np.ones(3)*BoxSize/nsub_per_side,
                                   k_edges=b_k_bins,pos_units_mpcoverh=pos_fac,
                                   second=sub_mesh[1],third=sub_mesh[2],
    ...
    print_status(comm,start_time,'Done measuring B from subbox %d' % ind)
